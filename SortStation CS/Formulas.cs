 ...
 * 2. Соответственно, вычисление нового выражения (две функции Evaluate подряд или вроде того) должно обходиться без инициализации
 * 3. Вместо работы со строкой (Queue<char>), добавить классы вроде Token, Variable, и т.п. и работать с ними
 *   (Это также уберёт ограничение на величину вычисляемого выражения)
 * 4. Убрать этот костыль с циклом на 100 повторений в методе Modify
 * 5. Изменить название класса на более подходящее
 * 6. Добавить возможность выбора пользователем, от скольких переменных будет выражение (соответственно изменить сигнатуру метода Evaluate)
 * 7. Добавить приоритеты функциям, чтобы не писать их вечно в скобках
 */
